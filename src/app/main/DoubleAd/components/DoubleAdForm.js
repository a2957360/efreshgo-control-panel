...
function DoubleAdForm(props) {
	...
	const handleImageDelete = async (img, language) => {
		const query = {
			deleteImages: [img]
		};
	...
	const canSubmit = () => {
		return selected !== undefined && newAd !== undefined;
	};
	...
	const handleSave = async () => {
		...
		if (result) {
			setAlert({ show: true, message: id === 'new' ? '新增广告成功！' : '广告信息修改成功！' });
			setTimeout(() => {
				setAlert({ show: false, message: '' });
				setLoading(false);
				history.push(`/homepage/doubleads`);
			}, 2000);
		}
	};
		...
		return (
			<div className="p-16 sm:p-24 max-w-2xl">
				<Typography className="m-20 normal-case flex items-center sm:mb-12" role="subtitle1" color="inherit">
					<span className="mx-4 text-16">上传广告图片 (图片规格 585*480)</span>
				</Typography>
				{newAd.Zh === '' ? (
					<label
						htmlFor="button-file"
					...
					>
						...
						<img className={classes.imagePreview} src={newAd.Zh} alt="ad_zh" />
					</div>
				)}
				...
				{newAd.En === '' ? (
					...
					>
						...
						<img className={classes.imagePreview} src={newAd.En} alt="ad_en" />
					</div>
				)}
