...
function RecipeForm(props) {
	const { form, setForm, tabIdx, setAlert, loading, setLoading } = props;
	// 中文富文本
	const [editorState_Zh, setZhEditorState] = useState(
		BraftEditor.createEditorState(form.cookbookDescription.Zh)
	);
	// 英文富文本
	const [editorState_En, setEnEditorState] = useState(
		BraftEditor.createEditorState(form.cookbookDescription.En)
	);
	...
	const handleZhEditorChange = editorState => {
		...
		setForm({...form, cookbookDescription: {...form.cookbookDescription, Zh: content}});
	};

	const handleEnEditorChange = editorState => {
		setEnEditorState(editorState);
		const content = editorState.toRAW();
		setForm({...form, cookbookDescription: {...form.cookbookDescription, En: content}});
	};
	...
	if (loading) {
		...
		return (
			<div className="p-16 sm:p-24 max-w-2xl">
				...
				{tabIdx === 2 && (
					<div>
						<ImageUpload form={form} setForm={setForm} setLoading={setLoading} />
					</div>
				)}
