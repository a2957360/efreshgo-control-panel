...
const ImagesUpload = props => {
	...
	const handleUpload = async e => {
		...
		const imgArray = _.concat([...form.cookbookImages], imageURL);
		setForm(_.set({ ...form }, `cookbookImages`, imgArray));
		setLoading(false);
	};
	...
	const handleImageDelete = async img => {
		...
		if (result.data.message === 'success') {
			const imgArray = _.pull([...form.cookbookImages], img);
			setForm(_.set({ ...form }, `cookbookImages`, imgArray));
		}
	};
	...
	if (loading) {
		...
		return (
			<div className="flex justify-center sm:justify-start flex-wrap -mx-8">
				...
				{form.cookbookImages.map((img, index) => {
					return (
						<div
							key={`cookbook_images_${index.toString()}`}
							className={clsx(
						...
						>
							...
							<img className={classes.imagePreview} src={img} alt="cookbook_images" />
						</div>
					);
				})}
			</div>
