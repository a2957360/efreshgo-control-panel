...
function Refund(props) {
	...
	const getOrderDetail = async () => {
        ...
        if(data.data.length === 0){
            window.alert('未找到该订单')
            setTimeout(() => {
                props.history.push('/order/refunds')
            }, 1000)
        }else{
            setForm(data.data[0]);
        }
		...
		return (
			<>
				...
				<FusePageCarded
					...
					header={<RefundHeader {...props} form={form} setAlert={setAlert}/>}
					// contentToolbar={
					// 	<Tabs
					// 		value={tabIdx}
					...
					// 	</Tabs>
					// }
					content={<RefundDetail {...props} form={form} setForm={setForm} setAlert={setAlert} />}
					innerScroll
				/>
			</>
		);
