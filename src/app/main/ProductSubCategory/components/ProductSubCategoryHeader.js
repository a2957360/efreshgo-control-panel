...
const ProductCategoryHeader = props => {
	...
	const canSubmit = () => {
		const isComplete =
			form.categoryTitle.Zh !== '' &&
			form.categoryImages.length !== 0 &&
	...
	const handleSave = async () => {
		...
		if (result.data.message === 'success') {
			setAlert({
				show: true,
				message: categoryNumber === 'new' ? '新增商品分类成功！' : '商品分类信息修改成功！'
			});
			setTimeout(() => {
				setAlert({ show: false, message: '' });
				history.push(`/product/subcategories`);
			}, 2000);
		}
	};
			...
			<div className="flex flex-col items-start max-w-full">
				<FuseAnimate animation="transition.slideRightIn" delay={300}>
					<Typography
						...
						component={Link}
						role="button"
						to="/product/subcategories"
						color="inherit"
					>
						<Icon className="text-20">arrow_back</Icon>
						<span className="mx-4 text-16">商品二级分类管理</span>
					</Typography>
				</FuseAnimate>
					...
					<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
						<FuseAnimate animation="transition.slideLeftIn" delay={300}>
							<Typography className="text-16 sm:text-20 truncate">
								{form.categoryTitle.Zh === '' ? '新增商品二级分类' : form.categoryTitle.Zh}
							</Typography>
						</FuseAnimate>
						<FuseAnimate animation="transition.slideLeftIn" delay={300}>
